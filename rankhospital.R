rankhospital <- function(state, outcome, num = "best") {
  ...
  minDeath<-min(SubsetOutcomeCM[,Coloutcome], na.rm=TRUE)
  HospitalRow<-subset(SubsetOutcomeCM, SubsetOutcomeCM[,Coloutcome]==minDeath)
  HospitalName<-as.character(HospitalRow[order(HospitalRow[,2],2),2])
}